...
- `PROJECTION-CURRENTNESS-THEORY-v0.md` — Round-1 foundations;
- `ROUND-1-PROJECTION-CURRENTNESS-AND-EVIDENCE-DESTRUCTIVE-TESTS.md`.

## v1 theorem profile

- authority is claim-local, not a universal lattice;
- supersession is a strict partial order only within stable claim epochs under owner-policy side conditions;
- claim migration is separate from supersession;
- evidence frontiers are unique/input-order independent on finite or maximality-satisfying query snapshots;
- conflict is preserved on the current frontier rather than hidden by aggregation;
- conservative projection refinement preserves adequacy only for fixed target judgement/observation/time/epoch;
- projection refinement is distinct from new evidence/currentness updates;
- standing derivations transport only when all bridge premises remain stable;
- multi-generation history forbids unwitnessed cross-epoch evidence splicing.

No NCT v1, Capability Calculus v1 or NDF0-NDF5 reopen condition fired. NDF6 remains NOT ADMITTED.
